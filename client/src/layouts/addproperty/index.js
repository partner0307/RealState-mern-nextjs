...
class Overview extends Component {
      ...
      if (this.state.selectedphoto) {
        ...
        await axios.post(apiConstants.PROPERTY_PHOTO_UPLOAD, data).then((res) => {
          if (res.data.message === 'uploaded') {
            let method;
            method = 'POST';
            let APIURL = apiConstants.PROPERTY_REGISTRATION_URL;
  ...
  maxSelectFile = (event) => {
    ...
    if (files.length < 2) {
      ...
      Object.keys(event.target.files).map((key) => {
        ...
        this.setState({ uploadImage: item.name });
        if (imageNameValidation.test(item.name.slice(0, -4))) {
  ...
  // handleImageChangedoc = (e) => {
  // //  console.log(e.target.files[0]);
  //   var url = URL.createObjectURL(e.target.files[0]);
  //   // setDocsrc(url);
  //   // setSelectedFilesdoc(e.target.files[0]);
  // };
  deleteFile = async (e) => {
                ...
                <Grid item xs={12} md={6} xl={4}>
                  ...
                  <MDBox mb={2}>
                    <MDInput
                      ...
                      value={this.state.propertyImages}
                      fullWidth
                    />
                  </MDBox>
                </Grid>
