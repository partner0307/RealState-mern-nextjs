...
class Overview extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      name: '',
      plave: '',
      SocialData: JSON.parse(this.props.availavility),
      socialArray: [
        {
          id: 1,
          name: 'Website',
      ...
      completiondate: this.props.completiondate,
      ...
      propertyImagesedit: [],
      // selectedphoto: [],
      photoUrlbanner: apiConstants.BASE_URL + '/images/' + this.props.bannerImg,
      selectedFilesdoc: [],
      selectedFiles: [],
      selectedimg: [],
  ...
  handleSubmit = async (e) => {
    e.preventDefault();
    // if (this.validate()) {
    var existinimages = JSON.parse(this.props.developmentImage);
    // console.log(existinimages);
    if (this.state.selectedimg || existinimages.length > 1) {
      ...
      await axios.post(apiConstants.DEVELOPMENTBANNER_UPLOAD, datasingleupload).then(async () => {
        await axios.post(apiConstants.DEVELOPMENT_PHOTO_UPLOAD, data).then(() => {
          ...
          let method;
          method = 'PUT';
          let APIURL = apiConstants.UPDATE_DEVELOPMENT;
          ...
          fetch(APIURL, requestOptions)
            ...
            .then(() => this.setState({}))
            .then(sweetalertsuccess('Development updated succesfully'))
  ...
  handleImageChangeSingle = async (e) => {
    ...
    if (this.checkMimeType(e)) {
      ...
      await this.setState({ bannerImg: e.target.files[0] });
      if (this.state.selectedphotoSingle) {
        const reader = new FileReader();
        reader.onload = async (event) => {
          await this.setState({ photoUrlbanner: event.target.result });
        };
  ...
  render() {
    ...
    var data = JSON.parse(this.props.developmentImage);
    var propertyphotos = [];
    for (var i = 0; i < data.length; i++) {
      propertyphotos.push(
        <img
          className="editpropertyimg"
          src={this.state.photoUrl + JSON.parse(this.props.developmentImage)[i]}
        />
      );
    }
    return (
        ...
        <MDBox
          ...
          <Grid container spacing={6}>
            <Grid item xs={12} md={6} xl={3}>
              ...
              Banner image
              <div>
                <img className="bammerArea" src={this.state.photoUrlbanner} />
              </div>
              <MDBox mb={2}>
                <MDInput
                  type="file"
...
/> */}
          ...
          <Grid container spacing={6}>
            <Grid item xs={12} md={6} xl={12}>
              ...
              <div>
                <button className="addbutton btn mb-4" type="button" onClick={this.handleAddSocial}>
                  <span>
